...
const CartManagementPage = () => {
    ...
    const updateQuantity = async (cartItemId, newQuantity) => {
        ...
        try {
            const token = getAuthToken();
            await axios.patch(`http://localhost:5000/cart/items/${cartItemId}`, { quantity: newQuantity }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const removeItem = async (cartItemId) => {
        try {
            const token = getAuthToken();
            await axios.delete(`http://localhost:5000/cart/items/${cartItemId}`, { headers: { Authorization: `Bearer ${token}` } });
